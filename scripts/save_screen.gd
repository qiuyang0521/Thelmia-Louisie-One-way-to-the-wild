...
func _on_slot_pressed(slot: int) -> void:
	...
	# 恢复地图状态：当前节点 id、地图随机种子、地图库索引、暴露度、有机物、金币与骰子
	...
	SaveMgr.gold_count = data.get("gold_count", SaveMgr.GOLD_INITIAL)
	SaveMgr.set_dice_from_save(data.get("dice_tiers", []), data.get("dice_spent", 0))
